  ...
  return (
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.brand}>✂ ARUN SALOON</div>
        {!token ? (
          <>
            <div style={styles.tagline}>WALK-IN BOOKING</div>
            <h1 style={styles.h1}>Apna Token<br />Lo.</h1>
            <p style={styles.subtext}>Apna WhatsApp number daalo aur saloon mein jagah pakad lo!</p>
            ...
            <div style={styles.inputRow}>
              ...
              <input
                ...
                onKeyPress={e => e.key === 'Enter' && bookSlot()}
                style={styles.input}
              />
            </div>
            {error && <div style={styles.error}>{error}</div>}
            <button onClick={bookSlot} disabled={loading} style={styles.primaryBtn}>
              {loading ? 'Booking...' : 'BOOK MY SLOT →'}
            </button>
            <p style={styles.footerText}>No spam. Tokens reset every day at midnight.</p>
          </>
        ) : (
          <>
            <div style={styles.confirmedBadge}>● CONFIRMED</div>
            <div style={styles.tokenLabel}>AAPKA TOKEN</div>
            <div style={styles.tokenNumber}>#{token.number}</div>
            <div style={styles.infoRow}>
              <div style={styles.infoBox}>
                <div style={styles.infoBoxLabel}>POSITION</div>
                <div style={styles.infoBoxValue}>#{token.pos}</div>
              </div>
              <div style={styles.infoBox}>
                <div style={styles.infoBoxLabel}>APPROX WAIT</div>
                <div style={styles.infoBoxValue}>{token.wait}</div>
              </div>
            </div>
            <div style={styles.callBox}>
              <div style={styles.callBoxLabel}>SLOT TIMING JAANNE KE LIYE CALL KAREIN</div>
              <div style={styles.callBoxNumber}>{token.ownerPhone}</div>
            </div>
            <button onClick={() => { setToken(null); setPhone('') }} style={styles.secondaryBtn}>
              Check Your Token
            </button>
            <p style={styles.footerText}>Tokens har raat 12 baje reset hote hain</p>
          </>
        )}
      </div>
    </div>
